...
vec4 quintic4(vec4 t) { return t * t * t * (t * (t * 6.0 - 15.0) + 10.0); }
...
void main() {
    ...
    float distortionStrength = perlin3d(vec3(position.x * uDistortionFrequency, position.y * uDistortionFrequency, uTime * 0.001));
    vec3 displacementPosition = position * (uDisplacementFrequency + distortionStrength);
    float perlinStrength = perlin4d(vec4(displacementPosition, uTime * 0.001)) * uDisplacementStrength;
